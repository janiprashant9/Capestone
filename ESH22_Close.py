...
scaled_data = scaler.fit_transform(df[['CLOSE(H)']])
...
prediction = pd.DataFrame(testPredict_inverse, columns=['Predictions']).to_csv('ESH22_Predictions1.csv')
actual = pd.DataFrame(testY_inverse, columns=['Actual']).to_csv('ESH22_actual.csv')
